...
#define RESET_LOW() (GPIOB->BSRRH = 0x10000000) // set Reset to low
#define RESET_HIGH() (GPIOB->BSRRL = 0x00001000) // set Reset to high

// Alert on port B, bit 10
#define ALERT_READ (GPIOD->IDR & 0x0400) // read Alert
//=============================================================================
...
//	// Reset on port B, bit 11 
//	GPIOB->CRH &= 0xFFFF0FFF; // set push-pull output for Reset pin 
//	GPIOB->CRH |= 0x00001000; // RESET_LOW();
...
//-----------------------------------------------------------------------------
etError SHT3X_SetAlertLimits(
	ft humidityHighSet, ft temperatureHighSet, 
...
static ft SHT3X_CalcTemperature(u16t rawValue)
{
	// calculate temperature [°C] // T = -45 + 175 * rawValue / (2^16-1) 
...
static ft SHT3X_CalcHumidity(u16t rawValue)
{
	// calculate relative humidity [%RH] // RH = rawValue / (2^16-1) * 100
	//return 100.0f * (ft)rawValue / 65535.0f;
	return 105.0f * (ft)rawValue / 65535.0f;
}

//----------------------------------------------------------------------------- 
static u16t SHT3X_CalcRawTemperature(ft temperature)
{
	// calculate raw temperature [ticks] // rawT = (temperature + 45) / 175 * (2^16-1)
